...
async function getWorkspaceById(id, userId) {
  ...
  if (!workspaces) {
    throw new Error('Workspace not found', 404);
  }
  return workspaces;
}
async function updateWorkspace(id, data, userId) {
  ...
  if (workspaces.ownerId !== userId) {
    throw new Error('You do not have access to update this workspace', 403);
  }
...
async function deleteWorkspace(id, userId) {
  ...
  if (workspaces.ownerId !== userId) {
    throw new Error('You do not have access to delete this workspace', 403);
  }
